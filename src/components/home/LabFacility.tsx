...
export function LabFacility() {
  ...
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-grid-dark [background-size:76px_76px] [mask-image:radial-gradient(ellipse_75%_70%_at_50%_15%,#000,transparent_78%)]"
      />
      <div
        aria-hidden="true"
      ...
      <Container className="relative">
        ...
        <ul
          className={cn(
            'grid gap-px overflow-hidden rounded-2xl bg-white/10',
        ...
        >
          {DEPARTMENTS.map((d, i) => (
            ...
            >
              <p className="font-mono text-[12px] tabular-nums text-brand-400">
                {String(i + 1).padStart(2, '0')}
              </p>
              <h3 className="mt-4 text-[20px] font-bold tracking-[-0.02em] text-white">{d.name}</h3>
              <p className="mt-3 text-[14.5px] leading-relaxed text-slate-400">{d.description}</p>
            </Reveal>
          ))}
        </ul>
        ...
        {/* ---------- Instruments ---------- */}
        <div className="mt-14">
          <h3 className="flex items-center gap-2.5 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-500">
            <Cpu className="h-4 w-4 text-brand-400" strokeWidth={2.2} aria-hidden="true" />
            Installed analysers
          </h3>
          ...
          <ul className="mt-5 divide-y divide-white/[0.08] border-y border-white/[0.08]">
            {INSTRUMENTS.map((inst, i) => (
              ...
              >
                <div>
                  <p className="text-[17px] font-bold leading-tight tracking-[-0.02em] text-white">
                    {inst.make} <span className="text-brand-300">{inst.model}</span>
                  </p>
                  <p className="mt-1.5 text-[13px] text-slate-400">{inst.role}</p>
                  <p className="mt-1 text-[11.5px] font-bold uppercase tracking-[0.12em] text-slate-600">
                    {inst.department}
                  </p>
                </div>
                <ul className="flex flex-wrap gap-2">
                  {inst.covers.map((c) => (
                    <li
                      key={c}
                      className="rounded-md bg-white/[0.06] px-2.5 py-1.5 text-[12.5px] text-slate-300 ring-1 ring-inset ring-white/10"
                    >
                      {c}
                    </li>
                  ))}
          ...
          <p className="mt-7 max-w-2xl text-[13px] leading-relaxed text-slate-500">
